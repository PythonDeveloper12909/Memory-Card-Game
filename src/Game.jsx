...
function Game() {
  ...
  useEffect(() => {
    const timerID = setTimeout(() => {
      if (
        matched.length >= matchsize &&
        emojis[matched.at(-1)] === emojis[matched.at(-2)]
      ) {
  ...
  const flip = (index) => {
    setFlipped((prev) =>
      prev.includes(index) || prev.length >= matchsize
        ? prev
        : [...prev, index],
    );
  ...
  const matchcheck = (index) => {
    setMatched((prev) =>
      prev.length >= matchsize || prev.includes(index)
        ? prev
        : [...prev, index],
    );
  };
        ...
        {woncheck === false ? (
          <>
            <div className="score_display">
              ...
              <h3 className="timer">{format(timer)}</h3>
            </div>
            <div className="grid">
              {cards.map((card, index) => (
                <div
                  ...
                  key={index}
                >
                  <span className="front" onClick={() => matchcheck(index)}>
                    ⭐
                  </span>
